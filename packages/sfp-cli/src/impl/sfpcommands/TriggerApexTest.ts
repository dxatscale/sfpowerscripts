...
import { ExtendedTestOptions, RunAllTestsInPackageOptions } from "@dxatscale/sfpowerscripts.core/lib/apextest/ExtendedTestOptions"
...
export default class TriggerApexTest {
  ...
  async exec() {
    ...
    if (this.testLevel === TestLevel.RunAllTestsInOrg.toString()) {
      ...
      this.testLevel === "RunAggregatedTests"
    ) {
      if (this.packages == null || this.packages.length === 0) {
        throw new Error(
          "At least one package is required to run aggregated Apex tests"
        );
      }
      ...
      this.testLevel === TestLevel.RunApexTestSuite.toString()
    ) {
      ...
      this.testLevel === TestLevel.RunSpecifiedTests.toString()
    ) {
    ...
    if (
      (this.isValidateIndividualClassCoverage ||
        this.isValidatePackageCoverage) &&
      this.testLevel !== ExtendedTestOptions.RunAllTestsInPackage.toString()
    ) {
      throw new Error(
    ...
    if (
      ...
      for (const sfpPackage of sfpPackages) {
        const packageTestCoverage = new PackageTestCoverage(sfpPackage, this.getCoverageReport(outputdir), new ConsoleLogger(), conn);
        let result = await packageTestCoverage.validateTestCoverage(this.coveragePercent);

        if (!result.result) {
          isCoverageFailure = true;
          console.log(`${sfpPackage.package_name} package does not meet coverage requirements. ${result.message}`);
        }
      }
  ...
  private getCoverageReport(outputDir: string): any {
    let testCoverageJSON = fs
      .readFileSync(
        path.join(outputDir, "test-result-codecoverage.json")
      )
      .toString();
