...
jest.mock('../../src/sfdxwrappers/TriggerApexTestImpl', () => {
    class TriggerApexTestImpl {
        public constructor(target_org: string, project_directory: string, private testOptions: TestOptions) {}
        exec = jest.fn().mockReturnValueOnce(Promise.resolve(result));
...
describe('Given an org, trigger apex tests,along with specified test and coverage options', () => {
    it('Run specific tests, but do not validate individual coverage for  classes', () => {
        ...
        fsextraMock
            .mockImplementationOnce((path: any, options: string | { encoding?: string; flag?: string }) => {
