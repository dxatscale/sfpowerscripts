...
describe("Given a PackageDependencyResolver", () => {
    ...
    it("should resolve package dependencies to current branch", async () => {
        const packageDependencyResolver = new PackageDependencyResolver(conn, projectConfig, [
            "candidate-management",
            "contact-management",
        ]);
        const resolvedProjectConfig = await packageDependencyResolver.resolvePackageDependencyVersions();

        let packageDescriptor = resolvedProjectConfig.packageDirectories.find(
            (dir) => dir.package === "candidate-management",
        );
        ...
        packageDescriptor = resolvedProjectConfig.packageDirectories.find(
            (dir) => dir.package === "contact-management",
        );
    ...
    it("should skip dependencies on a subscriber package version id", async () => {
        ...
        const packageDescriptor = resolvedProjectConfig.packageDirectories.find(
            (dir) => dir.package === "candidate-management",
        );
        const techFrameworkDependency = packageDescriptor.dependencies.find((dependency) =>
            dependency.package.startsWith("tech-framework"),
        );
    ...
    it("should throw if dependency package version cannot be found for current branch ", async () => {
        ...
        const packageDependencyResolver = new PackageDependencyResolver(conn, projectConfig, ["candidate-management"]);
        expect(() => {
            return packageDependencyResolver.resolvePackageDependencyVersions();
        }).rejects.toThrow();
    });
    ...
    it("should throw if there are no validated dependency package id", async () => {
        response = { records: [] };
        const packageDependencyResolver = new PackageDependencyResolver(conn, falseProjectConfig, [
            "contact-management",
        ]);
        expect(() => {
            return packageDependencyResolver.resolvePackageDependencyVersions();
