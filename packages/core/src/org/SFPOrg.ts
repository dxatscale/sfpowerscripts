...
export default class SFPOrg extends Org {
   ...
   * @param  {PackageMetadata} packageMetadata
   */
  public async updateArtifactInOrg(
    logger: Logger,
    packageMetadata: PackageMetadata
  ): Promise<string> {
    ...
    if (artifactId == null) {
      ...
      >(this.getConnection(), "SfpowerscriptsArtifact2__c", {
        Name: packageName,
    ...
    SFPLogger.log(
      COLOR_KEY_MESSAGE(
        `Updated Org with new Artifact ${packageName} ${
          packageMetadata.package_version_number
  ...
  private async getArtifactRecordId(
    packageMetadata: PackageMetadata
  ): Promise<string> {
    let installedArtifacts = await this.getInstalledArtifacts();
  ...
  public async getAllInstalled2GPPackages(): Promise<PackageDetails[]> {
    ...
    let records = await InstalledPackagesQueryExecutor.exec(
      this.getConnection()
    );
  ...
  public async getAllInstalledManagedPackages(): Promise<PackageDetails[]> {
    ...
    return installedPackages.filter(
      (installedPackage) => installedPackage.type === "Managed"
    );
  }
  /**
  ...
  public async listAllPackages() {
    if (this.isDevHubOrg()) {
      let records = await QueryHelper.query<PackageTypeInfo>(
        packageQuery,
        this.getConnection(),
        true
      );
      records.forEach((record) => {
        record.IsOrgDependent =
          record.ContainerOptions === "Managed"
            ? "N/A"
            : record.IsOrgDependent === true
            ? "Yes"
            : "No";
      });
