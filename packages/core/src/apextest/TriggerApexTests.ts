...
export default class TriggerApexTests {
  ...
  public async exec(): Promise<{
    ...
    try {
      ...
      //Write files
      fs.writeJSONSync(path.join(this.testOptions.outputdir,`test-result-${testRunIdResult.testRunId}.json`),jsonOutput,{spaces:4});
      fs.writeJSONSync(path.join(this.testOptions.outputdir,`test-result-${testRunIdResult.testRunId}-coverage.json`),jsonOutput.coverage.coverage,{spaces:4});
    
      //Write Junit Result no matter what
      SFPLogger.log(`Junit Report file available at ${path.join(this.testOptions.outputdir,`test-result-${testRunIdResult.testRunId}-junit.xml`)}`)
      let reportAsJUnitReport = new JUnitReporter().format(testResult);
      fs.writeFileSync(path.join(this.testOptions.outputdir,`test-result-${testRunIdResult.testRunId}-junit.xml`),reportAsJUnitReport);
      ...
      if (testResult.summary.outcome == "Failed") {
        ...
        testReportDisplayer.printTestSummary(
          coverageResults.packageTestCoverage
        );
  ...
  private async triggerTestSynchronously(
    ...
    let result = (await testService.runTestSynchronous(
      payload,
      true,
      null
    )) as TestResult;
  ...
  public async checkRunStatus(testRunId: string): Promise<any | undefined> {
    ...
    const testRunSummaryResults = (await this.conn.tooling.autoFetchQuery(
      testRunSummaryQuery
    )) as any;
