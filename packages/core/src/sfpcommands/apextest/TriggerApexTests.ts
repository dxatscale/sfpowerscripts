...
export default class TriggerApexTests {
    ...
    public async exec(): Promise<{
        ...
        try {
            ...
            try {
                ...
                if (testExecErrorMsg) throw new Error(testExecErrorMsg);
                else throw err;
            }

            let testReportDisplayer = new TestReportDisplayer(testReport, this.testOptions, this.fileLogger);
    ...
    private getTestReport(testId: string) {
        let test_report_json = fs
            .readFileSync(path.join(this.testOptions.outputdir, `test-result-${testId}.json`))
            .toString();
        return JSON.parse(test_report_json);
    }

    private getTestId(): string {
        let test_id = fs.readFileSync(path.join(this.testOptions.outputdir, 'test-run-id.txt')).toString();
    ...
    private getCoverageReport(): any {
        let testCoverageJSON = fs
            .readFileSync(path.join(this.testOptions.outputdir, 'test-result-codecoverage.json'))
            .toString();
