...
export default class PackageDiffImpl {
  ...
  public async exec(): Promise<boolean> {
    ...
    if (!isNullOrUndefined(this.project_directory)) {
      ...
      console.log(`Project directory being analysed ${this.project_directory}`);
    } else {
      project_config_path = "sfdx-project.json";
      git = simplegit();
      console.log(`Project directory being analysed ${process.cwd()}`);
    }
      ...
      if (this.sfdx_package == dir.package) {
        ...
        if (tag) {
          ...
          if (!isNullOrUndefined(config_file_path))
            console.log(`Checking for changes to ${config_file_path}`);

          console.log(`Checking for changes in source directory '${dir.path}'`);
          ...
          console.log(`Checking for changes to package version number in sfdx-project.json`);
