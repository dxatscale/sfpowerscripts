...
export default class ValidateTestCoveragePackageImpl {
    public constructor(
        ...
        private required_coverage: number,
    ...
    public async exec(command: string): Promise<void> {
        ...
        if (Number(result.result.coverage) < this.required_coverage)
            throw new Error(
                `Package Code Coverage is currently at ${result.result.coverage}, which is less than the required coverage ${this.required_coverage} `
            );
