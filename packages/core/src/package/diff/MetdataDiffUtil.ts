...
import { METADATA_INFO, SOURCE_EXTENSION_REGEX } from '../../metadata/MetadataInfo';
...
export default class MetadataDiffUtil {
    public static gitTreeRevisionTo: {
        revision: string;
        path: string;
    }[];

    public static async isFileIncludesContent(diffFile: DiffFileStatus, content: string): Promise<boolean> {
        let fileAsString = await git.show(['--raw', diffFile.revisionFrom]);
        let result = fileAsString.includes(content);
        return result;
    }

    public static async fetchFileListRevisionTo(revisionTo: string, logger: Logger) {
    ...
    public static async copyFile(filePath: string, outputFolder: string, logger: Logger) {
        ...
        let gitFiles = await this.getRelativeFiles(filePath);
