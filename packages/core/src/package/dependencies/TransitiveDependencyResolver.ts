...
export default class TransitiveDependencyResolver {
    ...
    private fillDepsTransitively(
        ...
        for (let pkg of pkgs) {
            ...
            let uniqueDependencies = [...new Set(dependenencies.map((objects) => JSON.stringify(objects)))].map(
                (tmpString) => JSON.parse(tmpString),
            );
            for (let j = 0; j < uniqueDependencies.length; j++) {
                ...
                //do a dedup again
                uniqueDependencies = [...new Set(uniqueDependencies.map((objects) => JSON.stringify(objects)))].map(
                    (tmpString) => JSON.parse(tmpString),
                );
            }
