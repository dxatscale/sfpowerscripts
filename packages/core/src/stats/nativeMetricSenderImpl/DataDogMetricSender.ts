...
export class DataDogMetricsSender extends NativeMetricSender
{
  ...
  public  sendGaugeMetric(
    ...
    try {
      ...
      this.nativeDataDogMetricsLogger.gauge(
        metric,
        value,
        transformedTags
      );
      ...
      SFPLogger.log(`Unable to transmit metrics for metric ${metric} due to`+error,LoggerLevel.TRACE,this.logger);
    }
  }
  ...
  public sendCountMetric(
    ...
    try {
      let transformedTags = this.transformTagsToStringArray(tags);
      this.nativeDataDogMetricsLogger.increment(
        metric,
        1,
        transformedTags
      );
