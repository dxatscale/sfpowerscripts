...
export default class ArtifactInstallationStatusUpdater {
  ...
  public static async updatePackageInstalledInOrg(
    ...
    try {
      ArtifactMigrator.migrateArtifacts(target_org);
      ...
      async (bail) => {
        ...
        if (artifactId == null) {
          cmdOutput = child_process.execSync(
            `sfdx force:data:record:create --json -s SfpowerscriptsArtifact2__c -u ${username}  -v "Name=${packageName} Tag__c=${packageMetadata.tag} Version__c=${packageMetadata.package_version_number} CommitId__c=${packageMetadata.sourceVersion}"`,
            { encoding: "utf8",stdio:"pipe"}
          );
        } else if (artifactId) {
          cmdOutput = child_process.execSync(
            `sfdx force:data:record:update --json -s SfpowerscriptsArtifact2__c -u ${username} -v "Name=${packageName} Tag__c=${packageMetadata.tag} Version__c=${packageMetadata.package_version_number} CommitId__c=${packageMetadata.sourceVersion}" -i ${artifactId}`,
            { encoding: "utf8" ,stdio:"pipe"}
          );
        }
