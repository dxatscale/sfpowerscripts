...
export default class ArtifactInstallationStatusUpdater {
    public static async updatePackageInstalledInOrg(
        ...
        try {
            ...
            SFPLogger.log(`Unable to update details about artifacts to the org: ${error}`, LoggerLevel.DEBUG, logger);
            return false;
        }
    }
    ...
    private static async updateArtifact(
        ...
        let artifactId = await ArtifactInstallationStatusUpdater.getRecordId(username, packageMetadata);
        ...
        return await retry(
            async (bail) => {
                ...
                SFPLogger.log(
                    COLOR_KEY_MESSAGE(
                        `Updating Org with new Artifacts ${packageName} ${packageMetadata.package_version_number} ${
    ...
    private static async getRecordId(username: string, packageMetadata: PackageMetadata): Promise<string> {
        let installedArtifacts = await InstalledAritfactsFetcher.getListofArtifacts(username);
