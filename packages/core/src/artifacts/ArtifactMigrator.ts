...
export default class ArtifactMigrator {
    ...
    public static async exec(username: string): Promise<void> {
        if (
            ...
            if (ArtifactMigrator.isSfpowerscriptsArtifact2Exist) {
                ...
                console.log(
                    'The custom object SfpowerscriptsArtifact__c will be deprecated in future release. Move to the new version of SfpowerscriptsArtifact to maintain compatibility.'
                );
        ...
        if (ArtifactMigrator.isSfpowerscriptsArtifact2Exist && ArtifactMigrator.isSfpowerscriptsArtifactExist) {
            if (
    ...
    private static async migrate(username) {
        ...
        fs.writeFileSync('SfpowerscriptsArtifact2SObjectTreeFile.json', JSON.stringify(recordsToImport));
            ...
            await retry(
                async (bail) => {
                    ...
                    if (importResult.status === 1) {
                        throw new Error(
                            'Failed to migrate records from SfpowerscriptsArtifact__c to SfpowerscriptsArtifact2__c'
                        );
                    } else {
    ...
    private static async querySfpowerscriptsArtifact(username): Promise<void> {
        try {
            ...
            let records = (
                await conn.autoFetchQuery(
                    'SELECT Id, Name, CommitId__c, Version__c, Tag__c FROM SfpowerscriptsArtifact__c'
                )
            ).records;
