  ...
  let readPackageChangelog: ReadPackageChangelog = (changelogFilePath: string) => {
    ...
    switch(path.basename(changelogFilePath)) {
      case "ESBaseCodeLWCChangelog":
        packageChangelog = fs.readJSONSync(
          path.join(resourcesDir, "ESBaseCodeLWCChangelog.json"),
          {"encoding": "UTF-8"}
        );
        break;
      case "ESBaseStylesLWCChangelog":
        packageChangelog = fs.readJSONSync(
          path.join(resourcesDir, "ESBaseStylesLWCChangelog.json"),
          {"encoding": "UTF-8"}
        );
        break;
      case "ESObjectsChangelog":
        packageChangelog = fs.readJSONSync(
          path.join(resourcesDir, "ESObjectsChangelog.json"),
          {"encoding": "UTF-8"}
        );
        break;
      case "ESSpaceMgmtLWCChangelog":
        packageChangelog = fs.readJSONSync(
          path.join(resourcesDir, "ESSpaceMgmtLWCChangelog.json"),
          {"encoding": "UTF-8"}
        );
        break;
      default:
