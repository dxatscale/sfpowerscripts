...
export default class IncrementBuildNumber extends SfpowerscriptsCommand {
    ...
    protected static flagsConfig = {
        segment: flags.string({
            description: messages.getMessage('segmentFlagDescription'),
            options: ['Major', 'Minor', 'Patch', 'BuildNumber'],
            default: 'BuildNumber',
        }),
        appendbuildnumber: flags.boolean({
            char: 'a',
            description: messages.getMessage('appendBuildNumberFlagDescription'),
            dependsOn: ['runnumber'],
            exclusive: ['segment', 'commitchanges'],
        }),
        ...
        runnumber: flags.string({
            char: 'r',
            description: messages.getMessage('runNumberFlagDescription'),
    ...
    public async execute() {
        try {
            ...
            if (this.flags.refname) {
                fs.writeFileSync(
                    '.env',
                    `${this.flags.refname}_sfpowerscripts_incremented_project_version=${result.versionNumber}\n`,
                    { flag: 'a' }
                );
