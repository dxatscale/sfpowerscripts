...
* **Display the package's version number as the pipeline build number(set\_build\_number)**

  If you are using .NEXT as the package version number, check this flag to enable the pipeline's build number to reflect the package version number generated by the platform.
...
* 9.0.0 Add support for packages with NEXT version as the build number, Also add additional package related info to metadata
